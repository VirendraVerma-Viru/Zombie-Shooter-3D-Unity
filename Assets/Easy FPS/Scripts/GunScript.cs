...
public class GunScript : MonoBehaviour {
	...
	*/
	void FixedUpdate(){
		 ...
		 */
		//if aiming
        if ((Input.GetAxis("Fire2") != 0 && !reloading && !meeleAttack) || isAiming)
        {
    ...
    public void AimingButtonPressed()
    {
        if (isAiming)
            isAiming = false;
        else
    ...
    void CheckTheShoot()
    {
        ...
        if (Physics.Raycast(ray, out hit, 100))
        {
            ...
            if (hit.collider.gameObject.GetComponent<Robot>())
                hit.collider.gameObject.GetComponent<Robot>().TakeDamage(10f);
